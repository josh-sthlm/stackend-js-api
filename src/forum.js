...
export type ListForumsResult = XcapJsonResult & {
...
export type ListEntriesResult = XcapJsonResult & {
	likes: LikeDataMap,
...
export function listEntries({
	...
	return async (dispatch: Dispatch) => {
		let deployProfile = await dispatch(getDeployProfile());
		if (deployProfile === DeployProfile.CASTLE && !!isQna) {
			let server = await dispatch(qnaApi.getServer()); // FIXME: hack
			return await dispatch(
				getJson({
					url: server + '/question/view',
 ...
 * @param threadId {number} Thread id.
 */
export function pinForumThread({ threadId }: { threadId: number }): Thunk<*> {
 ...
 * @param threadId {number} Thread id.
 */
export function closeForumThread({ threadId }: { threadId: number }): Thunk<*> {
 ...
 */
export function removeForumThreadEntry({ entryId }: { entryId: number }): Thunk<*> {
 ...
 */
export function pinForumEntry({ entryId }: { entryId: number }): Thunk<*> {
	return post({ url: '/forum/thread/pin', parameters: arguments });
}
...
/**
 * Vote thumbns up/down for a forum entry.
 *
 */
 ...
 * @param entryId
 */
export function getVoteSummary({ entryId }: { entryId: number }): Thunk<*> {
	return getJson({
...
export function getThreadEntryFromRedux({
	...
	if (!!id) {
		return find(flatten(Object.values(forumThreads.forums)), {
