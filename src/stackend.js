 ...
 * @type {string}
 */
export const STACKEND_COMMUNITY: string = 'stackend';

/**
 * Commmunity permalink used for stackend.com news and examples
 * @type {string}
 */
export const STACKEND_DOT_COM_COMMUNITY: string = 'stackend-com';

/**
 * Defintion of a community
 */
 ...
 * @type {string}
 */
export const COMMUNITY_MANAGER_CONTEXT: string = 'community';

/**
 * CommunityManager component class
 * @type {string}
 */
export const COMPONENT_CLASS: string = 'se.josh.xcap.community.CommunityManager';

/**
 * Community permalink reserverd for news and documentation on stackend.com
 * @type {string}
 */
export const STACKEND_COM_COMMUNITY_PERMALINK: string = 'stackend-com';

export type GetCommunityResult = XcapJsonResult & {
...
export type ValidateCommunityPermalinkResult = XcapJsonResult & {
	valid: boolean,
...
export type StoreCommunityResult = XcapJsonResult & {
...
/**
 * Get the communitys private settings that are not exposed to the frontend
...
export type RemoveCommunityResult = XcapJsonResult & {
	dataRemoved: boolean
};
...
export type SearchCommunityResult = XcapJsonResult & {
 ...
 */
export function getCurrentCommunity(communities: Array<Community>): Thunk<?Community> {
	return (dispatch: any) => {
 ...
 * @return {Community} may return null,
 */
export function _getCurrentCommunity(communities: Array<Community>, request: Request): ?Community {
	if (typeof communities === 'undefined' || communities === null || communities.length === 0) {
		return null;
	}

	let currentCommunityPermalink = _getCurrentCommunityPermalinkFromUrl(request);
	if (currentCommunityPermalink === null) {
		return null;
	}
...
export function _getCurrentCommunityPermalinkFromUrl(request: Request, config: Config): any {
	...
	let re = new RegExp('^' + request.contextPath + '(:?/stacks)?/([^/]+).*');
	let r = re.exec(p);
	if (!r) {
		return null;
	}

	let currentCommunityPermalink = r[2];
 ...
 * @returns {boolean}
 */
export function isCommunityModerator(community: ?Community, userId: number): boolean {
 ...
 * @returns {boolean}
 */
export function isCommunityAdmin(community: ?Community, userId?: ?number): boolean {
	if (
...
export type GetModulesResult = XcapJsonResult & {
 ...
 * @returns {Promise}
 */
export function getModules({ communityId }: { communityId: number }): Thunk<GetModulesResult> {
	return getJson({
...
export type GetModuleResult = XcapJsonResult & {
	module: ?Module,
	rule: ?ModuleRule,
	commentRule: ?ModuleRule,
	stats: ?ModuleStats
};
...
export type StoreModuleResult = XcapJsonResult & {
	module: ?Module
};
...
export function getComponentLabel(componentClass: string): string {
	let t = COMPONENT_CLASS_TO_MODULE_NAME[componentClass];
 ...
 * @param moduleType
 */
export function getComponentClassFromModuleType(moduleType: string) {
	let t = MODULE_TYPE_TO_COMPONENT_CLASS[moduleType];
	if (typeof t === 'undefined') {
		return 'Unknown';
	}
...
export type ListAdminUsersResult = XcapJsonResult & {
	users: Array<User>
};
 ...
 */
export function listAdminUsers({ privilege }: { privilege: number }): Thunk<ListAdminUsersResult> {
	return getJson({
 ...
 * @returns {Thunk<XcapJsonResult>}
 */
export function removeUser({ userId }: { userId: number }): Thunk<XcapJsonResult> {
	return post({
