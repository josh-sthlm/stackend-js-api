...
import { QnaSearchType, ContentType, getCategories, search as _search, QnaTypeConverter } from './index';
...
export function changeFilter({ filter, contentType, triggerSearch = true }: ChangeFilter): Thunk<void> {
...
function generateQueryObject(searchString = '', selectedFilters: any, availableFilters: any): any {
  ...
  const searchType = selectedFilters.searchType
    ? QnaTypeConverter(selectedFilters.searchType.substring(selectedFilters.searchType.lastIndexOf('/') + 1))
    : undefined;
